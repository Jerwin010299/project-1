...
from kivy.uix.floatlayout import FloatLayout  # Import FloatLayout for HomeScreen layout
...
class CaptureScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Top bar with language buttons and back button
        top_bar = BoxLayout(size_hint=(1, None), height=50, spacing=10, padding=10)

        # Back button on upper left
        self.back_button = Button(text='Back', size_hint=(0.2, 1))
        ...
        top_bar.add_widget(self.english_button)
        ...
        top_bar.add_widget(self.tagalog_button)
    ...
    def bind_buttons(self):
        # Bind upload and capture buttons to their respective methods
        self.upload_button.unbind(on_press=self.upload_image)
        self.upload_button.bind(on_press=self.upload_image)
        self.capture_button.unbind(on_press=self.capture_image)
        self.capture_button.bind(on_press=self.capture_image)
        print("[DEBUG] Buttons bound")

    def reset_state(self):
        print("[DEBUG] Resetting CaptureScreen state")
...
class AnalyzeScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Add Save button
        self.save_button = Button(text='SAVE', size_hint=(1, 1.5))
        self.save_button.bind(on_press=self.save_results)  # Bind to save_results method
    ...
    def show_recommendation(self, recommendation):
        ...
        if ("Organic Treatment" in recommendation or "Organiko" in recommendation) and \
           ("Non-Organic Treatment" in recommendation or "Hindi Organiko" in recommendation):
    ...
    def save_results(self, instance):
        """Saves the image and analysis results to a file."""
        ...
        try:
            ...
            with open(filepath, "w") as f:
                f.write(f"Analysis Result: {result_text}\n")
                f.write(f"Image Path: {image_filepath}\n")  # Save the image path for reference

            # Show a success message with scrollable label
            content_layout = BoxLayout(orientation='vertical', spacing=10)
            scroll = ScrollView(size_hint=(1, None), size=(200, 150))
            label = Label(text=f"Results saved to {filepath}", size_hint_y=None, text_size=(200, None), halign='center', valign='middle')
            ...
            button_box = BoxLayout(size_hint=(1, None), height=40, spacing=10)
            button_box.add_widget(close_button)
            content_layout.add_widget(button_box)

            popup = Popup(title="Save Successful",
            ...
            # After saving, navigate to saved images screen and refresh
            app = App.get_running_app()
            popup.bind(on_dismiss=lambda instance: self.go_to_saved_images())

        except Exception as e:
            # Show an error message
            popup = Popup(title="Save Error",
                          content=Label(text=f"Error saving results: {str(e)}"),
                          size_hint=(0.8, 0.3))
            popup.open()

    def go_to_saved_images(self):
        app = App.get_running_app()
        app.root.current = 'saved_images'
...
class HomeScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.start_button = Button(text='Start', size_hint=(0.2, 0.05),
                                   pos_hint={'center_x': 0.5, 'center_y': 0.5})
        self.start_button.bind(on_press=self.start_app)
        layout.add_widget(self.start_button)

        # Saved Images button at top right
        self.saved_images_button = Button(text='Saved Images', size_hint=(0.3, 0.05),
                                          pos_hint={'right': .95, 'top': .97})
        self.saved_images_button.bind
